...
- Every test runs through `run_test()` → `run_limited()`: a `perl` wrapper (macOS has no `timeout(1)`) that runs it in its own process group, kills the whole group after `TEST_TIMEOUT` seconds (`config.sh`), and reports `(timed out after Ns)` or `(crashed: SIGSEGV)` on the FAIL line. Tests run with stdin from `/dev/null`.
